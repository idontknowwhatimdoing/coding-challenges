...
// Brian wants to order something that Anna is allergic to, and they agree that Anna won't pay for that item.
...
fn money_spent(mut keyboards: Vec<i32>, mut drives: Vec<i32>, b: i32) -> i32 {
	keyboards.sort_unstable_by(|a, b| b.cmp(a)); // descending order
	drives.sort_unstable(); // ascending order

	let mut max = -1;
	let mut i = 0;
	let mut j = 0;
	while i < keyboards.len() {
		while j < drives.len() {
			if keyboards[i] + drives[j] > b {
				break; // prevent j from incrementing
			}
...
fn main() {
	...
	// let keyboards = vec![3, 1];
	// let drives = vec![5, 2, 8];
	// println!("{}", money_spent(keyboards, drives, 10));
}
